/* RoadCrew - js/client/schedules.js
   The client-service desk. Same screen as admin/schedules.js, with the write
   put back and the whole page scoped to one brand.
...
(function (R) {
  ...
  // This desk writes. The eleven client-service accounts are the only ones that
  // do - the admin checks the month, the promoters follow it, and the planning
  // happens here.
  var READ_ONLY = false;

  // Every account carries one brand and sees only that brand's work. A client
  // saved without one would land on an empty list with no way to explain itself,
  // so the Users screen refuses to create one - but the page still says so out
  // loud rather than rendering a blank screen if it ever happens.
  var MY_BRAND = me.brandId || '';

  function isMine(schedule) {
    return !!schedule && schedule.brandId === MY_BRAND;
  }

  // The scoped list, used by every read on the page. Conflict detection is the
  // one deliberate exception - see clashesFor.
  function mySchedules() {
    var all = R.db.schedules();
    var out = [];
    var i;
    for (i = 0; i < all.length; i++) {
      if (isMine(all[i])) { out.push(all[i]); }
    }
    return out;
  }
  ...
  function buildFilters() {
    // No brand filter here. The whole list is one brand already, so a picker
    // offering the other three would only ever return nothing.
  ...
  function filtered() {
    ...
    // Scoped at the source rather than filtered afterwards, so there is no path
    // through this function that can return another brand's row.
    var all = mySchedules();
    var out = [];
    var i;
  ...
  // A guard rather than a silent return, so a pasted link to another brand's
  // schedule gets a sentence instead of a drawer that never opens. Same shape as
  // the promoter app's blockedReason.
  function blockedReason(schedule) {
    if (!schedule) { return 'That schedule no longer exists.'; }
    if (!isMine(schedule)) { return 'That schedule belongs to another brand.'; }
    return '';
  }
  ...
  function openDrawer(id) {
    ...
    // Nothing on this page renders another brand's row, but a deep link, a stale
    // tab or the address bar can all still name one.
    if (id) {
      var refused = blockedReason(existing);
      if (refused) {
        U.toast(refused, 'danger');
        return;
      }
    }
    ...
    // The brand is fixed, not chosen. Rendered as a disabled one-option select
    // rather than removed, so the drawer still says which brand this schedule
    // belongs to - and so a desk cannot file work under a brand it does not hold.
    var myBrand = R.db.byId('brands', MY_BRAND);
    byId('d-brand').innerHTML = optionsFor(myBrand ? [myBrand] : [], MY_BRAND);
    byId('d-brand').disabled = true;
    draft.brandId = MY_BRAND;
  ...
  // across EVERY brand, deliberately. This is the one read on the page that is not
  // scoped, and it has to be: a promoter booked by the Jasmine desk is just as
  // unavailable to Kapal Api, and scoping here would let two desks book the same
  // person on the same day with neither able to see it. The warning names the dates
  // and the outlet, which the desk needs, and nothing about the other brand.
  ...
  function saveDraft() {
    ...
    var record = {
      id: draft.id || U.uid('s'),
      // MY_BRAND, not draft.brandId. The draft is already fixed to it, but this
      // literal is the last thing between the form and storage - it is the right
      // place to be certain rather than merely confident.
      brandId: MY_BRAND,
  ...
  function renderChrome() {
    ...
    // The brand under the name, because on this page it is not decoration - it is
    // the reason the list holds what it holds.
    var brandEl = byId('side-brand');
    if (brandEl) {
      var b = R.db.byId('brands', MY_BRAND);
  ...
  function wire() {
    ...
    var filters = ['f-region', 'f-subregion', 'f-promoter', 'f-status'];
    ...
    // No d-brand listener: the select is disabled and the brand is fixed to the
    // account's own. Nothing can change it, so nothing listens for it changing.
  ...
  function init() {
    ...
    // openDrawer carries the guard, so a link to another brand's schedule shows
    // the refusal here rather than being dropped without a word.
    var id = deepLinkId();
    if (id) {
      openDrawer(id);
    }

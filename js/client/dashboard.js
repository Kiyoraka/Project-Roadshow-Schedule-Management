/* RoadCrew - js/client/dashboard.js
   The client-service desk's Main. Artboards 02 and 08, the same as the admin
   original, with one difference: every figure on it comes from this account's
   brand and no other.
...
(function (R) {
  ...
  // Same shape as js/client/schedules.js. One brand per account, and this page
  // reads the database exactly once - in init() - so scoping it there scopes
  // every tile, the week strip, the feed and the coverage table at a stroke.
  ...
  function mySchedules() {
    ...
    for (i = 0; i < all.length; i++) {
      if (all[i] && all[i].brandId === MY_BRAND) { out.push(all[i]); }
    }
    return out;
  }
  ...
  function renderChrome() {
    ...
    // The brand under the name, because on this page it is the reason every
    // number reads the way it does.
  ...
  function init() {
    ...
    // The single seam. Note that "Promoters on duty today" therefore counts only
    // promoters on THIS brand's work - which is right for a dashboard, and is
    // deliberately the opposite of the conflict warning in the schedules drawer.
    // That one looks across every brand, because a promoter booked by another
    // desk is genuinely unavailable. Two different questions, two scopes.
    var all = mySchedules();
